You are helping me create a set of development tasks in my existing Tasks database in Thomas Frank’s Ultimate Brain 3.0.

Context:

- I already have a Tasks database.
- I already have a Project called “CalebHunter.dev” for my portfolio.
- I already have a Select property called “Dev Phase” with options like:
  - Discovery, Research, Design, Planning, Build, Testing, Deployment, Monitoring, Iteration
- Tasks already support relations to other tasks (the same relation used for sub-tasks / parent tasks). Use this existing relation field to represent dependencies. Do NOT create any new properties or databases.

Your job:

1. For each item in the list below, create a new task in the existing Tasks database.
2. For every created task:
   - Set Name to the “Task” value.
   - Set Project to “CalebHunter.dev”.
   - Set Dev Phase to the “Dev Phase” value (use the existing Dev Phase options; do not create new ones).
   - If I have a Work Type or similar property for classifying dev work, set it to “Dev Feature” (or the closest existing dev-feature type) unless my schema clearly suggests something better.
   - Put the “Notes” text into the Description/Notes field.
3. For dependencies:
   - If “Depends on” is “–”, leave the relation empty.
   - If “Depends on” names another task, use the existing task↔task relation field to link this task to that prerequisite task.
   - Treat the relation as “this task depends on that other task being done first.”
4. Do NOT modify existing tasks or properties. Only add new tasks and use the existing relation field.
...
- Do NOT modify any existing tasks or properties.
- Do NOT create any new properties or databases.
- Only create the new tasks above, set Dev Phase, Project, and other fields as requested, and use the existing task relation to link dependencies.
